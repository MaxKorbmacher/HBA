...
if (is.na(args[4])==T) {
  params = 0
} else if (is.na(args[4])==F) {
  params=read.csv(paste0(args[4]))
}
noquote("Predict.")
df$brainage=predict(object=m,df) # predict brain age
if (length(df$age)>0) {
  df$corrected_brainage= df$brainage + (df$age - (params$slope*df$age+params$intercept)) # correct brain age, if true age is present
